...
def run(plan, args):
    ...
    el_context = run_geth(plan, args, network_params, num_participants, final_genesis_timestamp, el_genesis_data)
...
def run_geth(plan, args, network_params, num_participants, final_genesis_timestamp, el_genesis_data):
    geth_prefunded_keys_artifact_name = plan.upload_files(static_files.GETH_PREFUNDED_KEYS_DIRPATH, name="geth-prefunded-keys")
    launcher = geth.new_geth_launcher(network_params["network_id"],
                                      el_genesis_data,
